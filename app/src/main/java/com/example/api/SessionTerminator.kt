...
object SessionTerminator {
    ...
    private suspend fun shrinkActiveFocusTimerInRTDB(context: Context, email: String) {
        withContext(Dispatchers.IO) {
            try {
                // Safeguard: Check if local timer/stopwatch is still running before resetting RTDB node
                val prefs = context.applicationContext.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
                val isRunning = prefs.getBoolean("timer_is_running", false) || prefs.getBoolean("timer_is_stopwatch_active", false) || prefs.getBoolean("is_paused", false)
                if (isRunning) {
                    Log.i(TAG, "Active session is still running locally. Skipping RTDB node shrink to prevent discarding active timer.")
                    return@withContext
                }
